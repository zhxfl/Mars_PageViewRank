...
int startMap(Spec_t* spec)
{
	...
	checkCudaErrors(cudaMalloc((void**)&d_inputKeys, h_inputKeysBufSize));
	checkCudaErrors(cudaMemcpy(d_inputKeys, h_inputKeys, h_inputKeysBufSize, cudaMemcpyHostToDevice));

	checkCudaErrors(cudaMalloc((void**)&d_inputVals, h_inputValsBufSize));
	checkCudaErrors(cudaMemcpy(d_inputVals, h_inputVals, h_inputValsBufSize, cudaMemcpyHostToDevice));

	checkCudaErrors(cudaMalloc((void**)&d_inputOffsetSizes, sizeof(int4)*h_inputRecordCount));
	...
	checkCudaErrors(cudaMalloc((void**)&d_interKeysSizePerTask, sizeof(int)*h_actualNumThreads));
	cudaMemset(d_interKeysSizePerTask, 0, sizeof(int)*h_actualNumThreads);

	int*	d_interValsSizePerTask = NULL;
	checkCudaErrors(cudaMalloc((void**)&d_interValsSizePerTask, sizeof(int)*h_actualNumThreads));
	cudaMemset(d_interValsSizePerTask, 0, sizeof(int)*h_actualNumThreads);

	int*	d_interCountPerTask = NULL;
	checkCudaErrors(cudaMalloc((void**)&d_interCountPerTask, sizeof(int)*h_actualNumThreads));
	...
	checkCudaErrors(cudaMalloc((void**)&d_psKeySizes, sizeof(int)*h_actualNumThreads));
	int h_allKeySize = prefexSum((int*)d_interKeysSizePerTask, (int*)d_psKeySizes, h_actualNumThreads);

	int *d_psValSizes = NULL;
	checkCudaErrors(cudaMalloc((void**)&d_psValSizes, sizeof(int)*h_actualNumThreads));
	int h_allValSize = prefexSum((int*)d_interValsSizePerTask, (int*)d_psValSizes, h_actualNumThreads);

	int *d_psCounts = NULL;
	checkCudaErrors(cudaMalloc((void**)&d_psCounts, sizeof(int)*h_actualNumThreads));
	...
	checkCudaErrors(cudaMalloc((void**)&d_interKeys, h_allKeySize));
	cudaMemset(d_interKeys, 0, h_allKeySize);
	
	char*	d_interVals = NULL;
	checkCudaErrors(cudaMalloc((void**)&d_interVals, h_allValSize));
	cudaMemset(d_interVals, 0, h_allValSize);

	int4*	d_interOffsetSizes = NULL;
	checkCudaErrors(cudaMalloc((void**)&d_interOffsetSizes, sizeof(int4)*h_allCounts));
	...
	checkCudaErrors(cudaMalloc((void**)&d_keyValOffsets, sizeof(int2)*h_actualNumThreads));
	cudaMemset(d_keyValOffsets, 0, sizeof(int2)*h_actualNumThreads);

	int*	d_curIndex = NULL;
	checkCudaErrors(cudaMalloc((void**)&d_curIndex, sizeof(int)*h_actualNumThreads));
...
void startGroup(Spec_t* spec)
{
	...
	checkCudaErrors(cudaMalloc((void**)&d_outputKeys, g_spec->interAllKeySize));
	checkCudaErrors(cudaMalloc((void**)&d_outputVals, g_spec->interAllValSize));
	checkCudaErrors(cudaMalloc((void**)&d_outputOffsetSizes, sizeof(int4)*g_spec->interRecordCount));
	...
	checkCudaErrors(cudaMalloc((void**)&g_spec->interKeyListRange, keyListRangeSize));
	checkCudaErrors(cudaMemcpy(g_spec->interKeyListRange, *h_outputKeyListRange, keyListRangeSize, cudaMemcpyHostToDevice));
...
void MapReduce(Spec_t *spec)
{
	...
	if (g_spec->workflow == MAP_GROUP)
	{
		...
		if (g_spec->outputToHost == 1)
		{
			...
			checkCudaErrors(cudaMemcpy(g_spec->outputKeyListRange, g_spec->interKeyListRange, sizeof(int2)*g_spec->outputDiffKeyCount, cudaMemcpyDeviceToHost));
                        checkCudaErrors(cudaFree(g_spec->interKeyListRange));
		}
		goto EXIT_MR;
	}
...
EXIT_MR:
	if (g_spec->outputToHost == 1)
	{
		...
		checkCudaErrors(cudaMemcpy(h_outputKeys, g_spec->outputKeys, g_spec->outputAllKeySize, cudaMemcpyDeviceToHost));
		checkCudaErrors(cudaMemcpy(h_outputVals, g_spec->outputVals, g_spec->outputAllValSize, cudaMemcpyDeviceToHost));
		checkCudaErrors(cudaMemcpy(h_outputOffsetSizes, g_spec->outputOffsetSizes, indexSize, cudaMemcpyDeviceToHost));

		checkCudaErrors(cudaFree(g_spec->outputKeys));
		checkCudaErrors(cudaFree(g_spec->outputVals));
		checkCudaErrors(cudaFree(g_spec->outputOffsetSizes));
