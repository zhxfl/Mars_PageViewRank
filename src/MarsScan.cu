/*$Id: MarsScan.cu 717 2009-11-09 10:39:00Z wenbinor $*/
/*
...
//    assert(g_numEltsAllocated == 0); // shouldn't be called 
    ...
    {       
        ...
        if (numBlocks > 1) 
        {
            checkCudaErrors(cudaMalloc((void**) &g_scanBlockSums[level++],
...
{	
	...
	checkCudaErrors( cudaMemcpy( h_outLast, d_outArr+numRecords-1, sizeof(int),
                                cudaMemcpyDeviceToHost) );
	int* h_inLast = ( int* )malloc( sizeof( int ) );
	checkCudaErrors( cudaMemcpy( h_inLast, d_inArr+numRecords-1, sizeof(int),
                                cudaMemcpyDeviceToHost) );
